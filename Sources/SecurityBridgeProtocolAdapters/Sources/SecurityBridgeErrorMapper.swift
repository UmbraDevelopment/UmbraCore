...
public enum SecurityBridgeErrorMapper {
  ...
  public static func mapToSecurityError(_ error: Error) -> UmbraErrors.Security.Protocols {
    ...
    if let bridgeError = error as? SecurityBridgeError {
      switch bridgeError {
      case .invalidInputType:
        return UmbraErrors.Security.Protocols.invalidInput(description: "Invalid input type provided to security bridge")
      case .mappingFailed:
        return UmbraErrors.Security.Protocols.operationFailed(reason: "Security error mapping failed")
      case .unsupportedErrorType:
        return UmbraErrors.Security.Protocols.unsupportedOperation(feature: "Error mapping for unsupported type")
      case .invalidConfiguration:
        return UmbraErrors.Security.Protocols.invalidConfiguration(description: "Security bridge configuration invalid")
      }
    }

    // Default fallback
    return UmbraErrors.Security.Protocols.operationFailed(reason: "Security operation failed: \(error.localizedDescription)")
  }
