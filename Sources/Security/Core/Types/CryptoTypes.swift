...
public struct CryptoServiceDto: Sendable {
  ...
  public typealias EncryptFunction = @Sendable (SecureBytes, SecureBytes) async -> Result<SecureBytes, SecurityProtocolError>
  
  /// Function type for decrypt operation
  public typealias DecryptFunction = @Sendable (SecureBytes, SecureBytes) async -> Result<SecureBytes, SecurityProtocolError>
  
  /// Function type for hash operation
  public typealias HashFunction = @Sendable (SecureBytes) async -> Result<SecureBytes, SecurityProtocolError>
  
  /// Function type for hash verification operation
  public typealias VerifyHashFunction = @Sendable (SecureBytes, SecureBytes) async -> Result<Bool, SecurityProtocolError>
...
public struct AnyCryptoService {
  ...
  public func encrypt(data: SecureBytes, using key: SecureBytes) async -> Result<SecureBytes, SecurityProtocolError> {
    await dto.encrypt(data, key)
  }
  
  /// Decrypts binary data using the provided key
  public func decrypt(data: SecureBytes, using key: SecureBytes) async -> Result<SecureBytes, SecurityProtocolError> {
  ...
  public func verifyHash(data: SecureBytes, expectedHash: SecureBytes) async -> Result<Bool, SecurityProtocolError> {
    await dto.verifyHash(data, expectedHash)
  }
}

/// A CryptoServiceAdapter for converting between different types used by crypto service implementations
