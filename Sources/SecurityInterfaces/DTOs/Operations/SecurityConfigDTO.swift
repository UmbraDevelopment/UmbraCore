...
public struct SecurityConfigDTO: Sendable, Equatable {
  ...
  /// Initialisation vector or nonce if required
  ...
  /// Full initialiser with all configuration options
  /// - Parameters:
  ///   - algorithm: The algorithm identifier (e.g., "AES-GCM", "RSA", "PBKDF2")
  ///   - keySizeInBits: Key size in bits
  ///   - initializationVector: Optional initialisation vector
  ...
  ///   - iv: Optional initialisation vector (if nil, one will be generated)
  ...
  /// - Parameter iv: The initialisation vector to use
  /// - Returns: A new configuration with the specified initialisation vector
