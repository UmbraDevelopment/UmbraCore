...
final class ModernXPCTests: XCTestCase {
  ...
  func testXPCErrorDescription() {
    ...
    XCTAssertEqual(
      XPCError.connectionFailed("Test").errorDescription,
      "[Connection] XPC connection failed: Test"
    )
    XCTAssertEqual(
      XPCError.messageFailed("Test").errorDescription,
      "[Message] Failed to send XPC message: Test"
    )
    XCTAssertEqual(
      XPCError.invalidMessage("Test").errorDescription,
  ...
  func testXPCConnectionProtocolImplementation() async {
    ...
    actor MockXPCConnection: XPCConnectionProtocol {
      ...
      func send(_: [String: Any], replyHandler _: ((xpc_object_t) -> Void)?) {
        sendCalled=true
      }
    }
