...
public func convertCoreToInterfaces(_ source: UmbraErrorsCore.ErrorSource?) -> Interfaces.ErrorSource? {
    ...
    return source as? Interfaces.ErrorSource
}

/// Convert a Interfaces.ErrorSource to UmbraErrorsCore.ErrorSource
public func convertInterfacesToCore(_ source: Interfaces.ErrorSource?) -> UmbraErrorsCore.ErrorSource? {
    // In this case, they're actually the same type at runtime but the compiler doesn't know that 
    // This function exists just to help the compiler with type checking
    return source as? UmbraErrorsCore.ErrorSource
}
