...
public struct SendableFileAttributes: Sendable, Equatable {
    ...
    public func value<T>(for key: FileAttributeKey, as type: T.Type = T.self) -> T? {
    ...
    public func toDictionary() -> [FileAttributeKey: Any] {
        ...
        for (key, wrappedValue) in storage {
            ...
            case .uint64(let value): result[key] = value
            case .int16(let value): result[key] = value
            case .double(let value): result[key] = value
            case .date(let value): result[key] = value
            case .data(let value): result[key] = value
            case .number(let value): result[key] = value
            }
        }
