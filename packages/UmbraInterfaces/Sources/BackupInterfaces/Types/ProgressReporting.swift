...
public struct BackupProgress: Sendable, Equatable {
    ...
    public static func initialising() -> BackupProgress {
        return BackupProgress(phase: .initialising, percentComplete: 0.0)
    }
    ...
    public static func finalising(percentComplete: Double = 0.9) -> BackupProgress {
        return BackupProgress(phase: .finalising, percentComplete: percentComplete)
    }
    ...
    public static func cleanup(percentComplete: Double = 0.95) -> BackupProgress {
        return BackupProgress(phase: .cleanup, percentComplete: percentComplete)
    }
    ...
    public var description: String {
        ...
        if let processedItems = processedItems, let totalItems = totalItems {
            components.append("\(processedItems)/\(totalItems) items")
        }
        
        if let processedBytes = processedBytes, let totalBytes = totalBytes {
            let formatter = ByteCountFormatter()
            formatter.allowedUnits = [.useAll]
            formatter.countStyle = .file
            components.append("\(formatter.string(fromByteCount: Int64(processedBytes)))/\(formatter.string(fromByteCount: Int64(totalBytes)))")
        }
...
public actor BackupProgressMonitor: BackupProgressReporter {
    ...
    public func currentProgress(for operation: BackupOperation) -> BackupProgress? {
        return _currentProgress[operation]
    }
    ...
    public func reportCancellation(for operation: BackupOperation) async {
        ...
        // Notify observers
        if let onOperationCancelled = onOperationCancelled {
    ...
    public func reportFailure(_ error: Error, for operation: BackupOperation) async {
        ...
        // Notify observers
        if let onOperationFailed = onOperationFailed {
    ...
    public func reportCompletion(for operation: BackupOperation) async {
        ...
        // Notify observers
        if let onOperationCompleted = onOperationCompleted {
