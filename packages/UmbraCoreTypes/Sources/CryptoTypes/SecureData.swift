...
public struct SecureData: Sendable, Equatable {
    ...
    public init(_ data: Data) {
        self.storage = ManagedSecureStorage(data)
    }
    ...
    public init<Bytes: Collection>(bytes: Bytes) where Bytes.Element == UInt8 {
        self.storage = ManagedSecureStorage(Data(bytes))
    }
    ...
    public init(capacity: Int) {
        self.storage = ManagedSecureStorage(Data(capacity: capacity))
    }
    ...
    public func withSecureAccess<R>(_ body: (Data) throws -> R) rethrows -> R {
        return try storage.withSecureAccess(body)
    }
    ...
    public func appending(_ other: SecureData) -> SecureData {
        ...
        self.withSecureAccess { selfData in
