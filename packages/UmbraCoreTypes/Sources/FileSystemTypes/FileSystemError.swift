...
public enum FileSystemError: Error, Sendable {
  ...
  public var localizedDescription: String {
    switch self {
      case let .fileNotFound(path, message):
        "File not found at '\(path)': \(message)"
      case let .permissionDenied(path, message):
        "Permission denied for '\(path)': \(message)"
      case let .fileAlreadyExists(path, message):
        "File already exists at '\(path)': \(message)"
      case let .directoryNotEmpty(path, message):
        "Directory not empty at '\(path)': \(message)"
      case let .insufficientStorage(path, message):
        "Insufficient storage for operation on '\(path)': \(message)"
      case let .invalidPath(path, message):
        "Invalid path '\(path)': \(message)"
      case let .general(path, message, code):
        "File system error (code: \(code)) for '\(path)': \(message)"
    }
  }
}
