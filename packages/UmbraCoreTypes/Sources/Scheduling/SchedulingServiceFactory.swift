...
public enum SchedulingServiceFactory {
    ...
    public static func createService(with calendar: Calendar) -> SchedulingServiceProtocol {
        return DefaultSchedulingService(calendar: calendar)
    }
}
...
private actor SchedulingActor {
    ...
    /// Internal storage for schedules (would be replaced with persistent storage in a real implementation)
    var schedules: [String: ScheduleDTO] = [:]
    /// Internal storage for tasks (would be replaced with persistent storage in a real implementation)
    var tasks: [String: ScheduledTaskDTO] = [:]
    /// Task update callbacks
    var taskUpdateCallbacks: [String: (taskID: String, callback: @Sendable (ScheduledTaskDTO) -> Void)] = [:]
    ...
    func getSchedules(withFrequency frequency: ScheduleDTO.Frequency) -> [ScheduleDTO] {
        return schedules.values.filter { $0.frequency == frequency }
    }
    ...
    func getTasks(forSchedule scheduleID: String) -> [ScheduledTaskDTO] {
        return tasks.values.filter { $0.scheduleID == scheduleID }
    }
    ...
    func registerCallback(for taskID: String, callback: @Sendable @escaping (ScheduledTaskDTO) -> Void) -> String {
...
private final class DefaultSchedulingService: SchedulingServiceProtocol {
    ...
    init() {
        ...
        self.actor = SchedulingActor(calendar: calendar)
    }
    ...
    public func createSchedule(_ schedule: ScheduleDTO) async -> Result<ScheduleDTO, UmbraErrors.ErrorDTO> {
    ...
    public func updateSchedule(_ schedule: ScheduleDTO) async -> Result<ScheduleDTO, UmbraErrors.ErrorDTO> {
    ...
    public func deleteSchedule(withID scheduleID: String) async -> Result<Void, UmbraErrors.ErrorDTO> {
    ...
    public func getSchedule(withID scheduleID: String) async -> Result<ScheduleDTO, UmbraErrors.ErrorDTO> {
    ...
    public func listSchedules(withFrequency frequency: ScheduleDTO.Frequency) async -> Result<[ScheduleDTO], UmbraErrors.ErrorDTO> {
    ...
    /// - Returns: Success or error result
    public func enableSchedule(withID scheduleID: String) async -> Result<Void, UmbraErrors.ErrorDTO> {
        guard let schedule = await actor.getSchedule(withID: scheduleID) else {
            return .failure(UmbraErrors.ErrorDTO(
                identifier: UUID().uuidString,
                domain: UmbraErrors.ErrorDomain.scheduling,
    ...
    /// - Returns: Success or error result
    public func disableSchedule(withID scheduleID: String) async -> Result<Void, UmbraErrors.ErrorDTO> {
        guard let schedule = await actor.getSchedule(withID: scheduleID) else {
            return .failure(UmbraErrors.ErrorDTO(
                identifier: UUID().uuidString,
                domain: UmbraErrors.ErrorDomain.scheduling,
    ...
    public func createTask(_ task: ScheduledTaskDTO) async -> Result<ScheduledTaskDTO, UmbraErrors.ErrorDTO> {
    ...
    public func updateTask(_ task: ScheduledTaskDTO) async -> Result<ScheduledTaskDTO, UmbraErrors.ErrorDTO> {
    ...
    public func getTask(withID taskID: String) async -> Result<ScheduledTaskDTO, UmbraErrors.ErrorDTO> {
    ...
    public func listTasks(withStatus status: ScheduledTaskDTO.TaskStatus) async -> Result<[ScheduledTaskDTO], UmbraErrors.ErrorDTO> {
    ...
    public func listTasks(forSchedule scheduleID: String) async -> Result<[ScheduledTaskDTO], UmbraErrors.ErrorDTO> {
    ...
    public func cancelTask(withID taskID: String) async -> Result<Void, UmbraErrors.ErrorDTO> {
        ...
        if let startedAt = task.startedAt {
            duration = UInt64(Date().timeIntervalSince1970) - startedAt
        } else {
            duration = task.duration
        }
    ...
    public func calculateNextRunTime(for schedule: ScheduleDTO) async -> Result<Date, UmbraErrors.ErrorDTO> {
        ...
        guard let nextDate = calculationCalendar.nextDate(
            after: now,
            matching: components,
...
public extension UmbraErrors.ErrorDomain {
