...
public struct SecurityConfigOptions: Sendable, Equatable {
    ...
    public static func from(dictionary options: [String: Any]?) -> SecurityConfigOptions {
        ...
        for (key, value) in options {
            ...
            if ["algorithm", "keySize", "mode", "hashAlgorithm", "identifier", 
                "dataBase64", "dataHex", "keyBase64", "keyIdentifier", "signatureBase64"].contains(key) {
                continue
            }
    ...
    public func toDictionary() -> [String: Any] {
        ...
        if let algorithm = algorithm { result["algorithm"] = algorithm }
        if let keySize = keySize { result["keySize"] = keySize }
        if let mode = mode { result["mode"] = mode }
        if let hashAlgorithm = hashAlgorithm { result["hashAlgorithm"] = hashAlgorithm }
        if let identifier = identifier { result["identifier"] = identifier }
        if let dataBase64 = dataBase64 { result["dataBase64"] = dataBase64 }
        if let dataHex = dataHex { result["dataHex"] = dataHex }
        if let keyBase64 = keyBase64 { result["keyBase64"] = keyBase64 }
        if let keyIdentifier = keyIdentifier { result["keyIdentifier"] = keyIdentifier }
        if let signatureBase64 = signatureBase64 { result["signatureBase64"] = signatureBase64 }
