...
public enum DateTimeError: Int, Error, Sendable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .invalidTimeZoneIdentifier:
            return "[\(domain):\(code)] The provided time zone identifier is invalid."
        case .dateParsingFailed:
            return "[\(domain):\(code)] Failed to parse the date string with the provided format."
        case .invalidDateComponents:
            return "[\(domain):\(code)] The provided date components cannot form a valid date."
        }
    }
}
