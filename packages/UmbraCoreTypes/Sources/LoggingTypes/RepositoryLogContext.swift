...
public struct RepositoryLogContext: LogContextDTO, Equatable {
  ...
  public init(
    ...
    if let locationPath = locationPath {
  ...
  public init(
    ...
    if let metadata = metadata {
      self.repositoryID = metadata.storage["repository_id"]?.valueString
      self.locationPath = metadata.storage["location"]?.valueString
      self.state = metadata.storage["state"]?.valueString
      self.operation = metadata.storage["operation"]?.valueString
    } else {
