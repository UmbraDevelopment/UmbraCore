...
public enum NotificationServiceFactory {
    ...
    /// - Returns: An object conforming to the NotificationServiceProtocol
    public static func createService(with notificationCenter: NotificationCenter) -> NotificationServiceProtocol {
        return DefaultNotificationService(notificationCenter: notificationCenter)
    }
}
...
private final class DefaultNotificationService: NotificationServiceProtocol {
    ...
    private func createDTO(from notification: Notification) -> NotificationDTO {
        ...
        let userInfo = (notification.userInfo as? [AnyHashable: Any])?.reduce(into: [String: String]()) { result, pair in
