...
public enum CryptoTestUtilities {
  ...
  public static func createTestKey(for algorithm: String) -> Data {
    switch algorithm.lowercased() {
      case "aes-256", "aes-256-gcm", "aes256gcm", "aes256-gcm", "aes256", "aes-256-cbc",
           "aes256cbc":
        createTestData(size: 32) // 256 bits = 32 bytes
      case "aes-128", "aes-128-gcm", "aes128gcm", "aes128", "aes-128-cbc", "aes128cbc":
        createTestData(size: 16) // 128 bits = 16 bytes
      case "chacha20poly1305", "chacha20-poly1305":
        createTestData(size: 32) // 256 bits = 32 bytes
      default:
        // Default to AES-256 key size
        createTestData(size: 32)
    }
  }
  ...
  public static func createTestIV(for algorithm: String) -> Data {
    switch algorithm.lowercased() {
      case "aes-256-gcm", "aes256gcm", "aes-128-gcm", "aes128gcm":
        createTestData(size: 12) // 96 bits = 12 bytes for GCM
      case "aes-256-cbc", "aes256cbc", "aes-128-cbc", "aes128cbc":
        createTestData(size: 16) // 128 bits = 16 bytes for CBC
      case "chacha20poly1305", "chacha20-poly1305":
        createTestData(size: 12) // 96 bits = 12 bytes
      default:
        // Default to GCM IV size
        createTestData(size: 12)
    }
  }
  ...
  public static func createEncryptionOptions(
    ...
    let mode: EncryptionMode=switch algorithm {
      case .aes256CBC:
        .cbc
      case .aes256GCM:
        .gcm
      case .chacha20Poly1305:
        .gcm // ChaCha20 doesn't have a direct mode in EncryptionMode
    }

    // Create appropriate EncryptionPadding based on algorithm
    let padding: EncryptionPadding=switch algorithm {
      case .aes256CBC:
        .pkcs7
