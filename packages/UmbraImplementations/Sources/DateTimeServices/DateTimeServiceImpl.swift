...
public actor DateTimeServiceImpl: DateTimeServiceProtocol {
   ...
   */
  public func timeIntervalBetween(
    from: DateTimeTypes.DateTimeDTO,
    to: DateTimeTypes.DateTimeDTO
  ) async -> TimeIntervalDTO {
  ...
  public func add(
    ...
    to date: DateTimeTypes.DateTimeDTO
  ) async -> DateTimeTypes.DateTimeDTO {
    date.adding(seconds: interval.seconds)
  }
  ...
  public func subtract(
    interval: TimeIntervalDTO,
    from date: DateTimeTypes.DateTimeDTO
  ) async -> DateTimeTypes.DateTimeDTO {
    date.subtracting(seconds: interval.seconds)
  }
  ...
  public func isBefore(
    _ date: DateTimeTypes.DateTimeDTO,
    _ otherDate: DateTimeTypes.DateTimeDTO
  ) async -> Bool {
    date.timestamp < otherDate.timestamp
  }
   ...
   */
  public func format(
    date: DateTimeTypes.DateTimeDTO,
    using format: DateTimeFormatDTO
  ) async -> String {
   ...
   */
  public func parse(string: String, using format: DateTimeFormatDTO) async -> DateTimeTypes
  .DateTimeDTO? {
    // For ISO8601 format, use the built-in method on DateTimeDTO
    if format.isPredefined && format.formatString == "ISO8601" {
   ...
   */
  private func formatWithImplementation(
    date: DateTimeTypes.DateTimeDTO,
    using format: DateTimeFormatDTO
  ) -> String {
    ...
    if let timeZoneIdentifier=format.timeZoneIdentifier {
      formatter
        ...
        TimeZone(identifier: self.timeZoneIdentifier) ?? TimeZone.current
    } else {
   ...
   */
  private func parseWithImplementation(
    string: String,
    using format: DateTimeFormatDTO
   ...
   */
  private func calculateTimestamp(
    year: Int,
    month: Int,
    day: Int,
    hour: Int,
