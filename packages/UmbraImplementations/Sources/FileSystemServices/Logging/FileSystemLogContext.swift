...
public struct FileSystemLogContext: LogContextDTO {
  ...
  public func withPath(_ path: String) -> FileSystemLogContext {
    let updatedMetadata=metadata.withPrivate(key: "path", value: path)
  ...
  public func withStatus(_ status: String) -> FileSystemLogContext {
    let updatedMetadata=metadata.withPublic(key: "status", value: status)
    return withUpdatedMetadata(updatedMetadata)
  }
  ...
  public func withMetadata(_ additionalMetadata: LogMetadataDTOCollection) -> Self {
    ...
    return FileSystemLogContext(
      operation: operation,
      path: metadata.getString(key: "path"),
      source: source,
      metadata: updatedMetadata,
      correlationID: correlationID,
      isSecureOperation: metadata.getString(key: "isSecureOperation") == "true"
    )
  }
}
