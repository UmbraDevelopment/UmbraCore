        ...
        if createParentDirectories {
            ...
            if !fileManager.fileExists(atPath: directoryPath) {
                do {
                    ...
                    await logError("Failed to create parent directories: \(error.localizedDescription)", context: logContext)
        ...
        do {
            ...
            if let nsError = error as NSError? {
                ...
                case NSFileWriteVolumeReadOnlyError:
                    fileSystemError = .readOnlyVolume
                default:
                    fileSystemError = .writeError(error.localizedDescription)
                }
            } else {
                fileSystemError = .writeError(error.localizedDescription)
            }
