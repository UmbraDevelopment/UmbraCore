...
public class CopyFileCommand: BaseFileSystemCommand, FileSystemCommand {
  ...
  public func execute(
    ...
    await logDebug("Starting file copy operation", context: logContext)
    ...
    do {
      // Copy the file
      try fileManager.copyItem(atPath: validSourcePath, toPath: normalisedDestinationPath)

      await logInfo(
        "Successfully copied file from \(validSourcePath) to \(normalisedDestinationPath)",
        context: logContext
      )
