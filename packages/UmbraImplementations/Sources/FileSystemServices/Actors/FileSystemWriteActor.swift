...
public actor FileSystemWriteActor: FileWriteOperationsProtocol {
    ...
    public init(logger: LoggingProtocol, rootDirectory: String? = nil) {
        ...
        self.operationQueue = DispatchQueue(label: "com.umbra.filesystem.write", qos: .userInitiated)
    ...
    public func createFile(at path: String, options: FileCreationOptions?) async throws -> String {
        ...
        do {
            ...
            await logger.error("Failed to create file: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func writeFile(data: Data, to path: String, options: FileWriteOptions?) async throws {
        ...
        do {
            ...
            await logger.error("Failed to write file: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func writeString(_ string: String, to path: String, encoding: String.Encoding, options: FileWriteOptions?) async throws {
        ...
        do {
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to write string to file: \(error.localizedDescription)", context: enhancedContext)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "writeString", path: path)
            await logger.error("Failed to write string to file: \(wrappedError.localizedDescription)", context: enhancedContext)
            throw wrappedError
        }
    }
    ...
    public func createDirectory(at path: String, options: DirectoryCreationOptions?) async throws -> String {
        ...
        do {
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to create directory: \(error.localizedDescription)", context: context)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "createDirectory", path: path)
            await logger.error("Failed to create directory: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func delete(at path: String) async throws {
        ...
        do {
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to delete file or directory: \(error.localizedDescription)", context: context)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "delete", path: path)
            await logger.error("Failed to delete file or directory: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func move(from sourcePath: String, to destinationPath: String, options: FileMoveOptions?) async throws {
        ...
        do {
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to move file or directory: \(error.localizedDescription)", context: enhancedContext)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "move", path: sourcePath)
            await logger.error("Failed to move file or directory: \(wrappedError.localizedDescription)", context: enhancedContext)
            throw wrappedError
        }
    }
    ...
    public func copy(from sourcePath: String, to destinationPath: String, options: FileCopyOptions?) async throws {
        ...
        do {
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to copy file or directory: \(error.localizedDescription)", context: enhancedContext)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "copy", path: sourcePath)
            await logger.error("Failed to copy file or directory: \(wrappedError.localizedDescription)", context: enhancedContext)
            throw wrappedError
        }
    }
}
