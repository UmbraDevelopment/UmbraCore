...
public actor FileSystemReadActor: FileReadOperationsProtocol {
    ...
    public func readFile(at path: String) async throws -> Data {
        ...
        do {
            ...
            let successContext = context
                .withStatus("success")
                .withUpdatedMetadata(context.metadata.withPublic(key: "dataSize", value: String(data.count)))
            ...
            await logger.error("Failed to read file: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func readFileAsString(at path: String, encoding: String.Encoding) async throws -> String {
        ...
        do {
            ...
            let successContext = enhancedContext
                .withStatus("success")
                .withUpdatedMetadata(enhancedContext.metadata.withPublic(key: "stringLength", value: String(string.count)))
            await logger.debug("Successfully read file as string", context: successContext)
            
            return string
        } catch let error as FileSystemError {
            // Re-throw already wrapped errors
            await logger.error("Failed to read file as string: \(error.localizedDescription)", context: enhancedContext)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "readFileAsString", path: path)
            await logger.error("Failed to read file as string: \(wrappedError.localizedDescription)", context: enhancedContext)
            throw wrappedError
        }
    }
    ...
    public func fileExists(at path: String) async -> Bool {
        ...
        do {
            ...
            await logger.warning("File existence check failed: \(error.localizedDescription)", context: context)
            return false
        }
    }
    ...
    public func listDirectory(at path: String) async throws -> [String] {
        ...
        do {
            ...
            let successContext = context
                .withStatus("success")
                .withUpdatedMetadata(context.metadata.withPublic(key: "itemCount", value: String(contents.count)))
            ...
            // Re-throw already wrapped errors
            await logger.error("Failed to list directory: \(error.localizedDescription)", context: context)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "listDirectory", path: path)
            await logger.error("Failed to list directory: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
    ...
    public func listDirectoryRecursively(at path: String) async throws -> [String] {
        ...
        do {
            ...
            let successContext = context
                .withStatus("success")
                .withUpdatedMetadata(context.metadata.withPublic(key: "itemCount", value: String(absolutePaths.count)))
            await logger.debug("Successfully listed directory contents recursively", context: successContext)
            
            return absolutePaths
        } catch let error as FileSystemError {
            // Re-throw already wrapped errors
            await logger.error("Failed to list directory recursively: \(error.localizedDescription)", context: context)
            throw error
        } catch {
            // Wrap and log other errors
            let wrappedError = FileSystemError.wrap(error, operation: "listDirectoryRecursively", path: path)
            await logger.error("Failed to list directory recursively: \(wrappedError.localizedDescription)", context: context)
            throw wrappedError
        }
    }
}
