...
extension FileSystemServiceImpl {
    ...
    public func createDirectory(
        ...
        if fileManager.fileExists(atPath: path.path) {
            ...
            if !isDir.boolValue {
                await logger.warning("Cannot create directory, file exists at path: \(path.path)", metadata: nil)
                throw FileSystemInterfaces.FileSystemError.pathAlreadyExists(path: path.path)
            } else if !withIntermediates {
                // Directory already exists, which is fine
                await logger.debug("Directory already exists at \(path.path)", metadata: nil)
                return
            }
        }
        ...
        do {
            ...
            await logger.debug("Created directory at \(path.path)", metadata: nil)
        } catch {
            await logger.error("Failed to create directory at \(path.path): \(error.localizedDescription)", metadata: nil)
    ...
    public func isDirectoryEmpty(at path: FilePath) async throws -> Bool {
        ...
        do {
            ...
            await logger.debug("Checked if directory is empty at \(path.path): \(isEmpty)", metadata: nil)
            
            return isEmpty
        } catch {
            await logger.error("Failed to check if directory is empty at \(path.path): \(error.localizedDescription)", metadata: nil)
