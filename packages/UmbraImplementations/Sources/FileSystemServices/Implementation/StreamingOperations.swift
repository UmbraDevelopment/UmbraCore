...
extension FileSystemServiceImpl {
    ...
    public func readDataInChunks(
        ...
        do {
            ...
            await logger.error("Failed to read file in chunks at \(path.path): \(error.localizedDescription)", metadata: nil)
    ...
    public func writeDataInChunks(
        ...
        do {
            ...
            await logger.error("Failed to write file in chunks at \(path.path): \(error.localizedDescription)", metadata: nil)
            throw FileSystemInterfaces.FileSystemError.writeError(
                path: path.path,
                reason: error.localizedDescription
            )
    ...
    public func writeFileInChunks(
        ...
        try await writeDataInChunks(to: path, overwrite: overwrite) {
            return await generator.nextChunk()
        }
    }
}
