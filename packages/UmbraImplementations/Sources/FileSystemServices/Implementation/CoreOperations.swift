...
extension FileSystemServiceImpl {
    ...
    public func remove(
        ...
        do {
            ...
            await logger.error("Failed to remove item at \(path.path): \(error.localizedDescription)", metadata: nil)
            throw FileSystemInterfaces.FileSystemError.writeError(
                path: path.path,
    ...
    public func copy(
        ...
        do {
            ...
            await logger.debug("Copied item from \(sourcePath.path) to \(destinationPath.path)", metadata: nil)
        } catch {
            await logger.error("Failed to copy from \(sourcePath.path) to \(destinationPath.path): \(error.localizedDescription)", metadata: nil)
            throw FileSystemInterfaces.FileSystemError.writeError(
                path: destinationPath.path,
                reason: error.localizedDescription
            )
    ...
    public func move(
        ...
        do {
            ...
            await logger.debug("Moved item from \(sourcePath.path) to \(destinationPath.path)", metadata: nil)
        } catch {
            await logger.error("Failed to move from \(sourcePath.path) to \(destinationPath.path): \(error.localizedDescription)", metadata: nil)
            throw FileSystemInterfaces.FileSystemError.writeError(
                path: destinationPath.path,
                reason: error.localizedDescription
            )
