...
public actor SecureFileOperationsImpl: SecureFileOperationsProtocol {
  ...
  public func setSecurePermissions(
    _ permissions: SecurityPermission,
    at path: FilePathDTO
  ) async throws {
  ...
  public func secureDelete(at path: FilePathDTO, options: SecureDeletionOptions?) async throws {
    ...
    for pass in 1...passes {
      ...
      await logger.debug(
        "Completed secure delete pass \(pass) of \(passes)",
   ...
   */
  public func verifyFileIntegrity(at path: FilePathDTO, against _: Data) async throws -> Bool {
