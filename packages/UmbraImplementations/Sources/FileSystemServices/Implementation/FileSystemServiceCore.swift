...
public actor FileSystemServiceImpl: FileSystemServiceProtocol {
    ...
    internal let operationQueueQoS: DispatchQoS.QoSClass
    
    /// Default buffer size for streaming operations (default: 64KB)
    internal let defaultBufferSize: Int
    
    /// Security options for file operations
    internal let securityOptions: SecurityOptions
    
    /// Whether to run operations in the background
    internal let runInBackground: Bool
    
    /// The logger instance for recording file operations
    internal let logger: any LoggingInterfaces.LoggingProtocol
    ...
    public func getMetadata(at path: FilePath) async throws -> FileSystemMetadata? {
        ...
        do {
            ...
            throw FileSystemInterfaces.FileSystemError.readError(path: path.path, reason: error.localizedDescription)
        }
    }
    ...
    public func isDirectory(at path: FilePath) async throws -> Bool {
        ...
        await logger.debug("Checked if path is directory at \(path.path): \(isDir.boolValue)", metadata: nil)
