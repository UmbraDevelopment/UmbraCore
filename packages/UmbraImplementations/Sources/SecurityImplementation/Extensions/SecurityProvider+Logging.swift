...
extension SecurityProviderImpl {
    ...
    func logOperationStart(operation: SecurityOperation, config: SecurityConfigDTO) async {
        ...
        let safeConfig = "Algorithm: \(config.algorithm), KeySize: \(config.keySize), Mode: \(config.mode?.rawValue ?? "none")"
    ...
    func logOperationSuccess(operation: SecurityOperation, duration: Double) async {
        ...
        await logger.info("Successfully completed security operation: \(operation.description)", metadata: metadata)
    }
