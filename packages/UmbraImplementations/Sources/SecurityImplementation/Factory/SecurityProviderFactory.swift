...
public enum SecurityProviderFactory {
  ...
  private final class CryptoServiceAdapter: CryptoServiceProtocol, Sendable {
     ...
     */
    func sign(data: SecureBytes, using key: SecureBytes) async -> Result<SecureBytes, SecurityProtocolError> {
      // Check for valid inputs
      guard !data.isEmpty else {
     ...
     */
    func verifySignature(_ signature: SecureBytes, for data: SecureBytes, using key: SecureBytes) async -> Result<Bool, SecurityProtocolError> {
      // Check for valid inputs
