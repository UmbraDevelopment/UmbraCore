...
final class SignatureService: SecurityServiceBase {
    ...
    init(logger: LoggingInterfaces.LoggingProtocol) {
        fatalError("This initializer is not supported. Use init(cryptoService:keyManagementService:logger:) instead.")
    }
            ...
            do {
                ...
                if let keyId = config.options["keyId"] {
                    ...
                    case .success(_):
                        ...
                        let signatureResult = await cryptoService.encrypt(data: inputData, using: config.options["key"].flatMap({ SecureBytes(base64Encoded: $0) }) ?? SecureBytes())
                        
                        switch signatureResult {
                        case .success(let generatedSignature):
    ...
    func verify(config: SecurityConfigDTO) async -> SecurityResultDTO {
        ...
        do {
            ...
            guard let signature = config.options["signature"].flatMap({ SecureBytes(base64Encoded: $0) }) else {
            ...
            if config.options["key"].flatMap({ SecureBytes(base64Encoded: $0) }) != nil {
                ...
                let calculatedSignatureResult = await cryptoService.encrypt(data: inputData, using: config.options["key"].flatMap({ SecureBytes(base64Encoded: $0) }) ?? SecureBytes())
                ...
                case .failure(let error):
                    throw SignatureError.cryptoError("Failed to calculate signature for verification: \(error)")
                }
            } else {
