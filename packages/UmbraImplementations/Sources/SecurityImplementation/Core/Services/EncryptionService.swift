...
final class EncryptionService: SecurityServiceBase {
    ...
    func encrypt(config: SecurityConfigDTO) async -> SecurityResultDTO {
        ...
        do {
            ...
            guard let inputDataString = config.options["data"],
                  let inputData = SecureBytes(base64Encoded: inputDataString) else {
                throw EncryptionServiceError.invalidInput("Missing or invalid input data for encryption")
            }
            
            guard let keyString = config.options["key"],
                  let key = SecureBytes(base64Encoded: keyString) else {
                throw EncryptionServiceError.invalidInput("Missing or invalid encryption key")
            }
            
            // Generate IV if not provided
            let iv: SecureBytes
            if let ivString = config.options["iv"],
    ...
    func decrypt(config: SecurityConfigDTO) async -> SecurityResultDTO {
        ...
        do {
            ...
            guard let ivString = config.options["iv"],
                  let iv = SecureBytes(base64Encoded: ivString) else {
    ...
    private func performEncryption(data: SecureBytes, key: SecureBytes, iv: SecureBytes) async throws -> SecureBytes {
        // Perform the encryption using the crypto service
        do {
            ...
            if let dataMethod = cryptoService as? HasDataEncryption {
                let result = await dataMethod.encrypt(data: data.extractUnderlyingData(), key: key.extractUnderlyingData())
    ...
    private func performDecryption(data: SecureBytes, key: SecureBytes, iv: SecureBytes) async throws -> SecureBytes {
        // Perform the decryption using the crypto service
        do {
            // Convert SecureBytes to Data if necessary based on the crypto service implementation
            if let dataMethod = cryptoService as? HasDataEncryption {
                let result = await dataMethod.decrypt(data: data.extractUnderlyingData(), key: key.extractUnderlyingData())
                return SecureBytes(data: result)
            }
...
private extension SecureBytes {
    func extractUnderlyingData() -> Data {
        ...
        return self.data()  // Directly return the data as it's not optional
    }
}
