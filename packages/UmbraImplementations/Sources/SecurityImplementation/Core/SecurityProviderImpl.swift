 ...
 */
private func createLogContext(for config: SecurityConfigDTO, operationID: String) -> SecurityLogContext {
  var metadata = LogMetadataDTOCollection()
  metadata = metadata.withPublic(key: "algorithm", value: config.encryptionAlgorithm.rawValue)
  metadata = metadata.withPublic(key: "dataSize", value: String(config.options?.metadata?["inputDataSize"] ?? "0"))
  metadata = metadata.withPublic(key: "hasKey", value: String(config.options?.metadata?["keyIdentifier"] != nil))
...
/**
 ...
 - Returns: A log context with appropriate metadata
 */
private func createOperationMetadata(_ operation: CoreSecurityTypes.SecurityOperation, _ operationID: String) -> SecurityLogContext {
  var metadata = LogMetadataDTOCollection()
...
public actor SecurityProviderImpl: SecurityProviderProtocol, AsyncServiceInitializable {
  ...
  public init(
    ...
    self.storageService = SecureStorageService(logger: logger)
    self.logger = logger
  }
  ...
  public func generateKey(config: SecurityConfigDTO) async throws -> SecurityResultDTO {
    ...
    do {
      ...
      let keySize: Int
      if let options = config.options, let keySizeStr = options.metadata?["keySize"],
  ...
  public func performSecureOperation(config: SecurityConfigDTO) async throws -> SecurityResultDTO {
    ...
    guard let operationTypeStr = config.options?.metadata?["operationType"],
          let operation = CoreSecurityTypes.SecurityOperation(rawValue: operationTypeStr) else {
      throw CoreSecurityError.configurationError("Missing or invalid operation type in configuration")
    }
  ...
  public func secureStore(config: SecurityConfigDTO) async throws -> SecurityResultDTO {
    ...
    do {
      ...
      // Store data securely using storage service - directly use bridge method that handles Data type
  ...
  private func extractKeyIdentifier(from config: SecurityConfigDTO) -> String? {
    return config.options?.metadata?["keyIdentifier"] as? String
  }
  ...
  public func generateRandom(config: SecurityConfigDTO) async throws -> SecurityResultDTO {
    ...
    let length: Int
    if case .generateRandom = CoreSecurityTypes.SecurityOperation(rawValue: config.options?.metadata?["operationType"] ?? "") {
      // Get the length from metadata
      if let lengthStr = config.options?.metadata?["length"], let parsedLength = Int(lengthStr) {
        length = parsedLength
      } else {
        length = 32  // Default length for random generation
      }
    } else if let lengthStr = config.options?.metadata?["length"], let parsedLength = Int(lengthStr) {
      length = parsedLength
    } else {
      length = 32  // Default to 32 bytes if no length specified
    }
  ...
  func generateKeyPair(
    ...
    do {
      ...
      guard let base64KeyData = randomResult.resultData, 
            let keyData = Data(base64Encoded: base64KeyData) else {
        throw CoreSecurityTypes.SecurityError.keyGenerationFailed(
  ...
  public func createSecureConfig(options: SecurityConfigOptions) async -> SecurityConfigDTO {
    ...
    if let operationStr = options.metadata?["operation"],
  ...
  private func createOperationConfig(
    ...
    return createSecurityConfig(operation: operation, options: options, data: data, key: key)
  }
