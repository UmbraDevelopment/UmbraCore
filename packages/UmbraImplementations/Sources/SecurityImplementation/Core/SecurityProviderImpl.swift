...
public actor SecurityProviderImpl: SecurityProviderProtocol {
     ...
     */
    internal let logger: LoggingInterfaces.LoggingProtocol
    ...
    public func encrypt(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await encryptionService.encrypt(config: config)
    }
    ...
    public func decrypt(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await encryptionService.decrypt(config: config)
    }
    ...
    public func generateKey(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await keyManagementService.generateKey(config: config)
    }
    ...
    public func secureStore(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await secureStorageService.secureStore(config: config)
    }
    ...
    public func secureRetrieve(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await secureStorageService.secureRetrieve(config: config)
    }
    ...
    public func secureDelete(config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await secureStorageService.secureDelete(config: config)
    }
     ...
     */
    public func generateRandomData(length: Int, config: SecurityConfigDTO) async -> SecurityResultDTO {
        return await keyManagementService.generateRandomData(length: length, config: config)
    }
    ...
    public func performSecureOperation(
        ...
        let result: SecurityCoreTypes.SecurityResultDTO
        
        switch operation {
        case .encrypt:
            result = await encryptionService.encrypt(config: config)
        case .decrypt:
            result = await encryptionService.decrypt(config: config)
        case .hash:
            result = await hashingService.hash(config: config)
        case .sign:
            result = await signatureService.sign(config: config)
        case .verify:
            result = await signatureService.verify(config: config)
        case .generateKey:
            result = await keyManagementService.generateKey(config: config)
        case .store:
            result = await secureStorageService.secureStore(config: config)
        case .retrieve:
            result = await secureStorageService.secureRetrieve(config: config)
        case .delete:
            result = await secureStorageService.secureDelete(config: config)
        case .custom, .deriveKey:
            ...
            result = SecurityCoreTypes.SecurityResultDTO(
                status: .failure,
                error: SecurityCoreTypes.SecurityError.unsupportedOperation("Operation not yet implemented")
            )
        }
