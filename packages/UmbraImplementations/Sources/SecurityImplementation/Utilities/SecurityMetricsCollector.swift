...
final class SecurityMetricsCollector {
  ...
  func recordOperationCompletion(
    ...
    if let avgDuration = averagePerformance(for: operation) {
      secureMetadata["avgDurationMs"] = PrivacyTaggedValue(value: Int(avgDuration), privacyLevel: .public)
    }
  ...
  func recordPerformanceAnomaly(
    ...
    var secureMetadata: [String: PrivacyTaggedValue] = [
      ...
      "exceededBy": PrivacyTaggedValue(value: String(format: "%.1f%%", (duration - threshold) / threshold * 100), privacyLevel: .public)
    ]
