...
public actor StandardCryptoService: CryptoServiceProtocol {
        ...
        switch keyResult {
          case let .success(keyData):
            ...
            if keyData.count != kCCKeySizeAES256 {
              await logger?.error(
                "Invalid key size: expected \(kCCKeySizeAES256), got \(keyData.count)",
                context: context
              )
            ...
            if let providedIV=options?.iv {
              ...
              if providedIV.count != expectedIVSize {
                await logger?.error(
                  "Invalid IV size: expected \(expectedIVSize), got \(providedIV.count)",
                  context: context
                )
  ...
  public func hash(
    ...
    let algorithm=(options?.algorithm as? String) == "SHA-512" ? HashingAlgorithm
      .sha512 : HashingAlgorithm.sha256
