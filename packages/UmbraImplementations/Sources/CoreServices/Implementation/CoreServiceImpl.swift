...
public actor CoreServiceImpl: CoreServiceProtocol {
   ...
   */
  public func start(options _: CoreStartupOptionsDTO?) async -> CoreOperationalStatus {
   ...
   */
  public func subscribeToEvents(filter _: CoreEventFilterDTO?) -> AsyncStream<CoreEventDTO> {
    ...
    return AsyncStream { continuation in
      ...
      continuation.onTermination={ [weak self] _ in
        Task { [weak self] in
          ...
          await removeEventContinuation(for: subscriptionID)
        }
      }
    }
  }
...
extension CoreError: LoggableError {
  public var loggableDescription: String {
    switch self {
      case let .initialisation(message):
        "Core initialisation failed: \(message)"
      case let .serviceNotAvailable(serviceName):
        "Service not available: \(serviceName)"
      case let .invalidState(message, currentState):
        "Invalid state: \(message) (current: \(currentState.rawValue))"
    }
  }
  ...
  public var privacyClassification: PrivacyClassification {
    ...
    switch self {
      case .initialisation:
        .private // May contain sensitive path information
