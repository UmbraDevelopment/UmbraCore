...
public class BaseConfigCommand {
    ...
    func logOperationSuccess(
        ...
        await logger.log(.info, "Configuration operation successful: \(operation)", context: enrichedContext)
    }
    ...
    func logOperationFailure(operation: String, error: Error, context: LogContextDTO) async {
        ...
        await logger.log(.error, "Configuration operation failed: \(operation)", context: enrichedContext)
    }
    ...
    private func deepMerge(
        ...
        for (key, overlayValue) in overlay {
            if let baseValue = base[key] {
                ...
                case (.dictionary(let baseDict), .dictionary(let overlayDict)):
                    // Recursively merge dictionaries
                    result[key] = .dictionary(deepMerge(base: baseDict, overlay: overlayDict))
                    
                case (.array(let baseArray), .array(let overlayArray)):
