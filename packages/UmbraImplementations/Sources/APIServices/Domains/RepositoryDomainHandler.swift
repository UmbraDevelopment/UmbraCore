...
public struct RepositoryDomainHandler: DomainHandler {
  ...
  public func execute<T: APIOperation>(_ operation: T) async throws -> Any {
    ...
    throw APIError
      .operationNotSupported("Unsupported repository operation: \(String(describing: T.self))")
  }
  ...
  public func supports(_ operation: some APIOperation) -> Bool {
    operation is RepositoryAPIOperation
  }
   ...
   */
  private func mapRepositoryStatus(_ status: RepositoryInterfaces.RepositoryStatus) -> APIInterfaces
  .RepositoryStatus {
    switch status {
      case .ready:
        .ready
