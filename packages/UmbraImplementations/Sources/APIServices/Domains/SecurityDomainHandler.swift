...
public struct SecurityDomainHandler: DomainHandler {
    ...
    public func execute<T: APIOperation>(_ operation: T) async throws -> Any {
        ...
        do {
            ...
            throw APIError.operationNotSupported("Unsupported security operation: \(String(describing: T.self))")
        } catch {
    ...
    public func supports<T: APIOperation>(_ operation: T) -> Bool {
        return operation is SecurityAPIOperation
    }
     ...
     */
    private func handleEncryptData(_ operation: EncryptDataOperation) async throws -> EncryptionResult {
        ...
        let securityOperation: SecurityOperation
        if let key = operation.key {
            securityOperation = .encrypt(data: operation.data, key: key)
        } else {
            securityOperation = .encrypt(data: operation.data, key: nil)
        }
    ...
    private func handleGenerateKey(_ operation: GenerateKeyOperation) async throws -> KeyGenerationResult {
        ...
        metadata["keySize"] = PrivacyMetadataValue(value: String(operation.keySizeInBits), privacy: .public)
        if let keyID = keyIdentifier {
            metadata["keyIdentifier"] = PrivacyMetadataValue(value: keyID, privacy: .private)
        }
     ...
     */
    private func handleStoreSecret(_ operation: StoreSecretOperation) async throws -> StoreSecretResult {
        // Log with privacy controls
        var metadata = PrivacyMetadata()
     ...
     */
    private func handleRetrieveSecret(_ operation: RetrieveSecretOperation) async throws -> SecureBytes {
        // Log with privacy controls
        var metadata = PrivacyMetadata()
