...
public actor APIServiceActor: APIServiceProtocol {
    ...
    public func getVersion() async -> APIVersionDTO {
        ...
        // Return the current version (in a real implementation, this would be fetched from a configuration)
    ...
    public func subscribeToEvents(filter: APIEventFilterDTO?) -> AsyncStream<APIEventDTO> {
        ...
        logger.debug(
            ...
            metadata: [
                ...
                "filter_types": .public(filter?.eventTypes?.map { $0.rawValue }.joined(separator: ", ") ?? "all")
            ]
        )
