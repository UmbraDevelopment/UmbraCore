...
public actor DefaultErrorHandler: ErrorHandlerProtocol {
    ...
    public func handle<E: Error>(
        ...
        logMetadata["errorType"] = .string(String(describing: type(of: error)))
        logMetadata["errorMessage"] = .string(error.localizedDescription)
        
        // Add domain information if available
        if let domainError = error as? any ErrorDomainProtocol {
            logMetadata["errorDomain"] = .string(String(describing: type(of: domainError).domain))
            logMetadata["errorCode"] = .string("\(domainError.code)")
        }
