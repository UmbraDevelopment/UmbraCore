...
public actor LoggingServiceAdapter: LoggingServiceProtocol {
    ...
    public func addDestination(_ destination: LogDestination) async throws {
        ...
        await debug("Added log destination: \(destination.identifier)", metadata: nil, source: "LoggingServiceAdapter")
    }
    ...
    public func removeDestination(withIdentifier identifier: String) async -> Bool {
        ...
        if destinations.removeValue(forKey: identifier) != nil {
            await debug("Removed log destination: \(identifier)", metadata: nil, source: "LoggingServiceAdapter")
            return true
        }
        return false
    }
    ...
    public func setMinimumLogLevel(_ level: UmbraLogLevel) async {
        ...
        // Log the change
        await debug("Set minimum log level to: \(level.rawValue)", metadata: nil, source: "LoggingServiceAdapter")
    }
