...
public struct PerformanceBenchmark {
  ...
  public init(
    ...
    logger=Logger(subsystem: "com.umbra.security", category: "Performance")
  }
  ...
  public func stop() -> Double {
    ...
    let duration=Double(endTime.uptimeNanoseconds - startTime.uptimeNanoseconds) /
      1_000_000 // Convert to milliseconds

    // Log the benchmark result
    logger
      .debug(
        "Benchmark: \(operationName) [\(category)] completed in \(duration, privacy: .public) ms"
      )
...
extension PerformanceBenchmarkable {
  /**
   ...
   */
  public var benchmarkCategory: String {
   ...
   */
  public func benchmarkMetadata(for _: String) -> [String: String] {
    [:]
  }
...
public struct PerformanceStatistics: Sendable {
  ...
  public func statisticsFor(_ operationName: String) -> [String: Double] {
    ...
    let durations=measurements.map(\.durationMs)
