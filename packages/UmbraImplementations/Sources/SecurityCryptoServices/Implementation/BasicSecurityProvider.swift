...
public struct BasicSecurityProvider: EncryptionProviderProtocol {
    ...
    public func encrypt(plaintext: Data, key: Data, iv: Data, config: SecurityConfigDTO) throws -> Data {
        ...
        guard status == kCCSuccess, let cryptorRef = cryptorRef else {
            throw SecurityProtocolError.cryptographicError("Failed to create encryption context with status \(status)")
        }
        ...
        guard status == kCCSuccess else {
            throw SecurityProtocolError.cryptographicError("Encryption update failed with status \(status)")
        }
        ...
        guard status == kCCSuccess else {
            throw SecurityProtocolError.cryptographicError("Encryption finalization failed with status \(status)")
        }
    ...
    public func decrypt(ciphertext: Data, key: Data, iv: Data, config: SecurityConfigDTO) throws -> Data {
        ...
        guard status == kCCSuccess, let cryptorRef = cryptorRef else {
            throw SecurityProtocolError.cryptographicError("Failed to create decryption context with status \(status)")
        }
        ...
        guard status == kCCSuccess else {
            throw SecurityProtocolError.cryptographicError("Decryption update failed with status \(status)")
        }
        ...
        guard status == kCCSuccess else {
            throw SecurityProtocolError.cryptographicError("Decryption finalization failed with status \(status)")
        }
