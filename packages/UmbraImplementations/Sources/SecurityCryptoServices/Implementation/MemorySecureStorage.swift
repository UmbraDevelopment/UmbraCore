...
public actor TestSecureStorageActor: SecureStorageProtocol {
  ...
  public func storeData(
    ...
    await logger.debug(
      "Storing data in test secure storage",
      metadata: nil,
      source: "TestSecureStorageActor"
    )
  ...
  -> Result<[UInt8], SecurityStorageError> {
    await logger.debug(
      "Retrieving data from test secure storage",
      metadata: nil,
      source: "TestSecureStorageActor"
    )
  ...
  -> Result<Void, SecurityStorageError> {
    await logger.debug(
      "Removing data from test secure storage",
      metadata: nil,
      source: "TestSecureStorageActor"
    )
  ...
  public func listDataIdentifiers() async -> Result<[String], SecurityStorageError> {
    await logger.debug(
      "Listing identifiers in test secure storage",
      metadata: nil,
      source: "TestSecureStorageActor"
    )
