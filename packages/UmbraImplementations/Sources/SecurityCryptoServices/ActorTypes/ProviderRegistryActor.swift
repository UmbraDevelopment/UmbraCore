...
public actor ProviderRegistryActor {
    ...
    public func registerProvider(
        ...
        if providerFactories[type] != nil {
            await logger.warning("Provider of type \(type) already registered, will be replaced", metadata: LogMetadata())
        }
        ...
        do {
            ...
            await logger.debug("Successfully verified provider: \(type) - \(provider.providerType.description)", metadata: LogMetadata())
        } catch {
            // Remove the factory if it fails verification
            providerFactories.removeValue(forKey: type)
            await logger.error("Provider factory verification failed: \(error.localizedDescription)", metadata: LogMetadata())
            throw error
        }
    }
    ...
    public func listAvailableProviderTypes() async -> [SecurityProviderType] {
        return Array(providerFactories.keys)
    }
    ...
    public func listAvailableProviders() async throws -> [EncryptionProviderProtocol] {
        ...
        for (type, factory) in providerFactories {
            do {
                ...
                await logger.debug("Successfully instantiated provider: \(type) - \(provider.providerType.description)", metadata: LogMetadata())
            } catch {
                failedTypes.append(type)
                await logger.warning("Failed to instantiate provider \(type): \(error.localizedDescription)", metadata: LogMetadata())
            }
        }
        ...
        if providers.isEmpty && !providerFactories.isEmpty {
            ...
            throw SecurityProtocolError.unsupportedOperation(name: "No encryption providers could be instantiated. Failed types: \(failedTypes.map { $0.description }.joined(separator: ", "))")
        }
    ...
    public func selectProvider(
        ...
        await logger.debug("Selecting provider for capabilities: \(effectiveCapabilities.map { $0.rawValue }.joined(separator: ", "))", metadata: LogMetadata())
        ...
        if let preferredType = configuration.preferredProviderType,
           let factory = providerFactories[preferredType] {
            do {
                let provider = try factory()
                await logger.debug("Using preferred provider: \(preferredType) - \(provider.providerType.description)", metadata: LogMetadata())
                return provider
            } catch {
                await logger.warning("Preferred provider \(preferredType) failed to instantiate: \(error.localizedDescription)", metadata: LogMetadata())
        ...
        if let provider = providers.first {
            await logger.debug("Selected provider: \(provider.providerType.description)", metadata: LogMetadata())
            return provider
        }
        
        // If fallback is allowed, try to find any working provider
        if configuration.allowFallbackProviders {
            await logger.warning("No provider found meeting all capabilities, attempting fallback", metadata: LogMetadata())
            
            // Try to get any working provider
            if let (_, factory) = providerFactories.first {
                do {
                    let provider = try factory()
                    await logger.warning("Using fallback provider: \(provider.providerType.description)", metadata: LogMetadata())
                    return provider
                } catch {
                    await logger.error("Fallback provider failed to instantiate: \(error.localizedDescription)", metadata: LogMetadata())
                }
            }
        }
        ...
        throw SecurityProtocolError.unsupportedOperation(
            name: "No provider meeting capabilities: \(effectiveCapabilities.map { $0.rawValue }.joined(separator: ", "))"
        )
    }
    ...
    private func selectProviderForFipsCompliance() async throws -> EncryptionProviderProtocol {
        ...
        if let factory = providerFactories[.ring] {
            do {
                let provider = try factory()
                await logger.debug("Selected FIPS-compliant provider: \(provider.providerType.description)", metadata: LogMetadata())
                return provider
            } catch {
                await logger.warning("FIPS-compliant provider .ring failed to instantiate: \(error.localizedDescription)", metadata: LogMetadata())
                // Continue to try other providers
            }
        }
        ...
        if let factory = providerFactories[.apple] {
            do {
                let provider = try factory()
                await logger.warning("Using Apple provider as fallback for FIPS compliance: \(provider.providerType.description)", metadata: LogMetadata())
                return provider
            } catch {
                await logger.error("Apple provider FIPS fallback failed to instantiate: \(error.localizedDescription)", metadata: LogMetadata())
            }
        }
