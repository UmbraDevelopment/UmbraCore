...
public actor SecureStorageActor {
    ...
    public init(
        ...
        self.logger = logger
        self.cryptoService = CryptoServiceActor(providerType: providerType, logger: logger)
        ...
        Task {
            await logger.info("Initialised SecureStorageActor with storage at: \(self.storageURL.path)", metadata: nil)
        }
    }
    ...
    public func storeKey(
        ...
        if FileManager.default.fileExists(atPath: keyURL.path) && !overwrite {
            await logger.warning("Key with identifier '\(identifier)' already exists and overwrite is false", metadata: nil)
        ...
        do {
            ...
            if let secError = error as? SecurityProtocolError {
                ...
                throw SecurityProtocolError.cryptographicError("Failed to store key: \(error.localizedDescription)")
            }
        }
    }
    ...
    public func retrieveKey(withIdentifier identifier: String) async throws -> SecureBytes {
        ...
        do {
            ...
            if let secError = error as? SecurityProtocolError {
                ...
                throw SecurityProtocolError.cryptographicError("Failed to retrieve key: \(error.localizedDescription)")
            }
        }
    }
    ...
    public func deleteKey(withIdentifier identifier: String) async throws {
        ...
        do {
            ...
            throw SecurityProtocolError.cryptographicError("Failed to delete key: \(error.localizedDescription)")
        }
    }
    ...
    public func rotateKey(
        ...
        if let dataItems = dataToReencrypt {
            ...
            let reencryptedItems = try await cryptoService.encryptBatch(dataItems: decryptedItems, using: newKey)
            
            await logger.info("Successfully rotated key and re-encrypted \(dataItems.count) data items", metadata: nil)
    ...
    private func getMasterKey() async throws -> SecureBytes {
        ...
        do {
            ...
            await logger.error("Failed to retrieve master key: \(error.localizedDescription)", metadata: nil)
            throw SecurityProtocolError.cryptographicError("Failed to retrieve master key: \(error.localizedDescription)")
        }
    }
