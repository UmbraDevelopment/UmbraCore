...
public actor CryptoServiceActor {
    ...
    public init(providerType: SecurityProviderType? = nil, logger: LoggingProtocol) {
        ...
        do {
            if let providerType = providerType {
                self.provider = try SecurityProviderFactory.createProvider(type: providerType)
            } else {
                self.provider = try SecurityProviderFactory.createBestAvailableProvider()
            }
            
            // Create default configuration for this provider
            self.defaultConfig = SecurityConfigDTO.aesEncryption(providerType: self.provider.providerType)
            ...
            self.provider = SecurityProviderFactory.createDefaultProvider()
            self.defaultConfig = SecurityConfigDTO.aesEncryption(providerType: .basic)
            
            Task {
                await logger.warning("Failed to create requested provider, falling back to basic provider: \(error.localizedDescription)", metadata: nil)
            }
        }
    }
    ...
    public func setProvider(_ type: SecurityProviderType) -> Bool {
        do {
            ...
            Task {
                await logger.error("Failed to change security provider: \(error.localizedDescription)", metadata: nil)
            }
            return false
        }
    }
