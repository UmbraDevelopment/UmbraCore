...
public actor CoreFileOperationsImpl: CoreFileOperationsProtocol {
  ...
  public func readFileAsString(
    ...
    do {
      ...
      guard let string=String(data: data, encoding: encoding) else {
        ...
        await logger.error(
          "Failed to decode file content with specified encoding",
          context: errorContext
        )
        throw error
      }
    ...
    do {
      ...
      if options?.append == true {
        ...
        try data.write(
          ...
          options: options?.atomicWrite == true ? .atomic : []
        )
      }
  ...
  public func getFileURLs(in path: String) async throws -> ([URL], FileOperationResultDTO) {
    ...
    do {
      ...
      let errorContext=createLogContext(["path": path, "error": "\(error)"])
      await logger.error(
        "Failed to get file URLs: \(error.localizedDescription)",
        context: errorContext
      )
      throw dirError
    }
  ...
  public func move(
    ...
    do {
      ...
      if fileManager.fileExists(atPath: destinationPath) && !shouldOverwrite {
        ...
        await logger.error(
          "Destination already exists and overwrite not allowed",
          context: errorContext
        )
        throw error
      }
    ...
    do {
      ...
      func enumerateDirectory(_ dirPath: String) throws -> [String] {
        ...
        for item in contents {
          ...
          if
            fileManager.fileExists(atPath: fullPath, isDirectory: &isItemDir) && isItemDir
              .boolValue
          {
