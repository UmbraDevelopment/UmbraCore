...
public final class PrivacyAwareLogFormatter: LogFormatterProtocol {
  ...
  public init(
    ...
    self.includePrivateDetails = includePrivateDetails ?? (environment != LoggingTypes.DeploymentEnvironment.production)
  ...
  public func format(_ entry: LoggingTypes.LogEntry) -> String {
    ...
    if includeMetadata {
      if let metadata = entry.metadata, let metadataStr = formatMetadata(convertToLogMetadata(metadata)), !metadataStr.isEmpty {
  ...
  public func withConfiguration(
    ...
    return PrivacyAwareLogFormatter(
      environment: self.environment,
      includePrivateDetails: self.includePrivateDetails,
  ...
  private func convertToMetadataDict(_ metadata: LogMetadata) -> [String: (value: Any, privacy: LogPrivacyLevel)] {
   ...
   */
  private func shouldRedactAutomatically(_ value: String) -> Bool {
  ...
  /**
   ...
   ```
   */
  public func configuredFor(dto: PrivacyAwareLogDTO) -> PrivacyAwareLogFormatter {
