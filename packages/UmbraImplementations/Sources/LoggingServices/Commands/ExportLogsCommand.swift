...
public class ExportLogsCommand: BaseCommand, LogCommand {
   ...
   */
  public func execute(context _: LoggingInterfaces.LogContextDTO) async throws -> Data {
    ...
    do {
      ...
      // Log success
      await logInfo(
        "Successfully exported \(exportedData.count) bytes of log data in \(options.format.rawValue) format"
      )
