...
public class WriteLogCommand: BaseCommand, LogCommand {
    ...
    do {
      ...
      if success {
        ...
        throw LoggingTypes.LoggingError
          .writeFailure("Failed to write log to destination '\(destination.name)'")
      }
  ...
  private func shouldWriteToDestination() async -> Bool {
    ...
    if
      destination.configuration.filterRules == nil || destination.configuration.filterRules?
        .isEmpty == true
    {
      return true
    }
   ...
   */
  private func applyRedactionRules(_ entry: LoggingInterfaces.LogEntryDTO) -> LoggingInterfaces
  .LogEntryDTO {
