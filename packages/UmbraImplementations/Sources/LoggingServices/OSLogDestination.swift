...
public struct OSLogDestination: LoggingTypes.LogDestination {
    ...
    private func extractPrivacySettings(from metadata: LogMetadata?) -> (messagePrivacy: LogPrivacy, metadataPrivacy: LogPrivacy) {
        ...
        let metadataPrivacy: LogPrivacy
        if metadataPrivacyStr == "public" {
            ...
            metadataPrivacy = .private // Default is private for metadata
        }
    ...
    public func write(_ entry: LoggingTypes.LogEntry) async throws {
        ...
        if let metadata = filteredMetadata, !metadata.asDictionary.isEmpty {
            ...
            case .public:
                ...
                case .public:
                    os_log("%{public}@ [Metadata: %{public}@]", log: osLog, type: type, entry.message, metadataStr)
                default:
                    os_log("%{public}@ [Metadata: %{private}@]", log: osLog, type: type, entry.message, metadataStr)
                }
            default:
                switch metadataPrivacy {
                case .public:
                    os_log("%{private}@ [Metadata: %{public}@]", log: osLog, type: type, entry.message, metadataStr)
                default:
                    os_log("%{private}@ [Metadata: %{private}@]", log: osLog, type: type, entry.message, metadataStr)
                }
            }
        } else {
