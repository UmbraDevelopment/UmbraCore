...
public struct BaseDomainLogger: DomainLogger {
    ...
    ///   - context: The context for the log
    ///   - message: Optional custom message
    public func logOperationStart<T: LogContextDTO>(context: T, message: String? = nil) async {
        let logMessage = message ?? "Starting operation"
        await logger.info(logMessage, metadata: context.toPrivacyMetadata(), source: context.getSource())
    }
    ...
    ///   - result: Optional result information
    ///   - message: Optional custom message
    public func logOperationSuccess<T: LogContextDTO, R: Sendable>(
        context: T, 
        result: R? = nil, 
        message: String? = nil
    ) async {
...
public struct ErrorLogger {
    ...
    public func createContextFromError(
        ...
        if let domainError = error as? CustomNSError {
            context.addPrivate(key: "errorDomain", value: String(describing: type(of: domainError).errorDomain))
...
public struct FileSystemLogger {
    ...
    public func logPath(
        ...
        switch level {
        case .trace:
            message = "Trace file operation: \(operation)"
        case .debug:
            message = "Debug file operation: \(operation)"
        case .info:
            message = "File operation: \(operation)"
        case .warning:
            message = "Warning during file operation: \(operation)"
        case .error:
            message = "Error during file operation: \(operation)"
        case .critical:
            message = "Critical error during file operation: \(operation)"
        }
        ...
        switch level {
        case .trace:
            await domainLogger.logger.trace(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        case .debug:
            await domainLogger.logger.debug(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        case .info:
            await domainLogger.logger.info(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        case .warning:
            await domainLogger.logger.warning(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        case .error:
            await domainLogger.logger.error(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        case .critical:
            await domainLogger.logger.critical(message, metadata: fsContext.toPrivacyMetadata(), source: fsContext.getSource())
        }
    }
}
