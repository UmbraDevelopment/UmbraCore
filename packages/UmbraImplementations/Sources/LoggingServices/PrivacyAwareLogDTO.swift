...
public struct PrivacyAwareLogDTO: LogContextDTO, Sendable {
     ...
     */
    public func with(metadata additionalMetadata: [String: (value: Any, privacy: LogPrivacyLevel)]) -> PrivacyAwareLogDTO {
        let combinedMetadata = self.extractRawMetadata()
    ...
    public func with(source: String?) -> PrivacyAwareLogDTO {
        return PrivacyAwareLogDTO(
            ...
            domainName: self.domainName,
            correlationID: self.correlationID,
            metadata: self.extractRawMetadata(),
            environment: self.environment
        )
    }
     ...
     */
    private static func createMetadataCollection(from dict: [String: (value: Any, privacy: LogPrivacyLevel)]) -> LoggingTypes.LogMetadataDTOCollection {
    ...
    public func toLogContext() -> LogContext {
        return LogContext(
            source: self.source ?? "UmbraCore",
            metadata: self.metadata.toPrivacyMetadata()
        )
    }
...
public struct PrivacyValueWrapper {
    ...
    public init(value: String, privacy: LogPrivacyLevel) {
        ...
        self.privacyClassification = privacy
    }
}
