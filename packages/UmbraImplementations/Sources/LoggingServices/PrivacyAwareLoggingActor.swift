...
public actor PrivacyAwareLoggingActor: PrivacyAwareLoggingProtocol {
  ...
  public nonisolated var loggingActor: LoggingActor {
    ...
    return LoggingActor(destinations: [], minimumLogLevel: .info)
  }
  ...
  public func log(_ level: LogLevel, _ message: String, context: LogContextDTO) async {
    ...
    let privacyContext: PrivacyAwareLogDTO
    if let existingPrivacyContext = context as? PrivacyAwareLogDTO {
      privacyContext = existingPrivacyContext
    } else {
      // Create a new PrivacyAwareLogDTO with the source from the context
      privacyContext = PrivacyAwareLogDTO(
  ...
  public func log(_ level: LogLevel, _ message: PrivacyString, context: LogContextDTO) async {
    ...
    let processedMessage = message.rawValue // In a real implementation, use message.processForLogging()
