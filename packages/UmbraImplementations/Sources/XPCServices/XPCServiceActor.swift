...
public actor XPCServiceActor: XPCServiceProtocol {
    ...
    public func start() async throws {
        ...
        do {
            ...
            throw XPCServiceError.connectionFailed("Failed to start XPC service: \(error.localizedDescription)")
        }
    }
    ...
    public func sendMessage<T: Sendable, R: Sendable>(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            do {
                ...
                let data: Data
                do {
                    data = try NSKeyedArchiver.archivedData(withRootObject: message, requiringSecureCoding: true)
                } catch {
                    throw XPCServiceError.messageEncodingFailed("Failed to encode message: \(error.localizedDescription)")
                }
                
                // Send the message
                proxy.handleMessage(data, forEndpoint: endpoint?.name ?? "default") { responseData, error in
                    if let error = error {
                        continuation.resume(throwing: XPCServiceError.handlerError("Remote handler error", error))
                        return
                    }
                    
                    guard let responseData = responseData else {
                        continuation.resume(throwing: XPCServiceError.responseDecodingFailed("No response data received"))
                        return
                    }
                    
                    do {
                        // Decode the response
                        guard let responseObject = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(responseData),
                              let response = responseObject as? R else {
                            throw XPCServiceError.responseDecodingFailed("Failed to decode response as expected type")
                        }
                        
                        continuation.resume(returning: response)
                    } catch {
                        continuation.resume(throwing: XPCServiceError.responseDecodingFailed("Failed to decode response: \(error.localizedDescription)"))
                    }
                }
            } catch {
    ...
    public func registerHandler<T: Sendable, R: Sendable>(
        ...
        let wrappedHandler = AnyXPCHandler { data in
            // Decode the message
            guard let messageObject = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data),
                  let message = messageObject as? T else {
    ...
    private func log(_ level: LogLevel, _ message: String) async {
        if let logger = logger {
            await logger.logMessage(level, message, context: .init(source: "XPCServiceActor"))
        }
    }
     ...
     */
    fileprivate func handleIncomingMessage(_ messageData: Data, forEndpoint endpointName: String) async throws -> Data {
 ...
 */
@objc private protocol XPCMessageHandler {
    /**
     ...
     */
    func handleMessage(_ message: Data, forEndpoint endpoint: String, withCompletion completion: @escaping (Data?, Error?) -> Void)
}
