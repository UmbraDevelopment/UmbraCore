...
public class AuthenticationServiceFactory {
    ...
    public static func createAuthenticationService(
        ...
        switch providerType {
        case .apple:
            provider = AppleAuthenticationProvider(securityProvider: securityProvider)
            
        case .ring(let parameters):
            provider = RingAuthenticationProvider(
    ...
    public static func createRingAuthenticationService(
        ...
        let parameters: RingAuthenticationProvider.Argon2Parameters
        
        switch targetEnvironment {
        case .server:
            parameters = RingAuthenticationProvider.Argon2Parameters.server
        case .mobile:
            parameters = RingAuthenticationProvider.Argon2Parameters.mobile
        case .minimal:
            parameters = RingAuthenticationProvider.Argon2Parameters.minimal
        }
