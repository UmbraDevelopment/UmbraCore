...
public actor DefaultCryptoServiceImpl: CryptoServiceProtocol {
  ...
  public func encrypt(
    ...
    let contextWithOptions: CryptoLogContext=if let algorithm=options?.algorithm {
      CryptoLogContext(
          ...
          guard let keyIDLengthByte=encryptedDataBytes.last else {
            let errorContext=CryptoLogContext(
              ...
              additionalContext: context.metadata.withPublic(key: "status", value: "failed")
                .withPublic(
                  key: "error",
                  value: "Invalid encrypted data format: missing key ID length"
                )
            )
          ...
          guard
            ...
            let errorContext=CryptoLogContext(
              ...
              additionalContext: context.metadata.withPublic(key: "status", value: "failed")
                .withPublic(
                  key: "error",
                  value: "Invalid encrypted data format: key ID length mismatch"
                )
            )
  ...
  private func storeData(
    ...
    await storeData(data: data, identifier: identifier)
  }
