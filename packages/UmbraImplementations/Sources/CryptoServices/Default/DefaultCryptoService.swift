...
/**
 ...
 Default implementation of CryptoServiceProtocol using a command pattern architecture.
 ...
 - Basic key derivation with secure parameters
 - Secure random generation using SecRandomCopyBytes
 - Event logging for security operations
...
public actor DefaultCryptoService: CryptoServiceProtocol {
  ...
  /**
   Initialises the default crypto service.
  ...
  public init(secureStorage: SecureStorageProtocol, logger: LoggingProtocol?=nil) {
    ...
    self.logger=logger
    commandFactory=CryptoCommandFactory(secureStorage: secureStorage, logger: logger)
  }
