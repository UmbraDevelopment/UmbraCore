...
public actor KeychainXPCService: KeychainServiceProtocol {
    ...
    public func start() async throws {
        ...
        do {
            ...
            throw KeychainError.serviceStartFailed("Failed to start XPC service: \(error.localizedDescription)")
        }
    }
    ...
    public func storePassword(
        ...
        do {
            try await sendRequest(request, to: .storePassword)
    ...
    public func retrievePassword(for account: String) async throws -> String {
        ...
        do {
            let response: KeychainStringResponse = try await sendRequest(request, to: .retrievePassword)
    ...
    public func deletePassword(for account: String) async throws {
        ...
        do {
            try await sendRequest(request, to: .deleteItem)
    ...
    public func storeData(
        ...
        do {
            try await sendRequest(request, to: .storeData)
    ...
    public func retrieveData(for account: String) async throws -> Data {
        ...
        do {
            let response: KeychainDataResponse = try await sendRequest(request, to: .retrieveData)
    ...
    /**
     Sends a request to the XPC service and returns the response.
     ...
     */
    private func sendRequest<T: Sendable, R: Sendable>(
        _ request: T,
    ...
    private func mapError(_ error: Error, operation: String) -> KeychainError {
        if let xpcError = error as? XPCServiceError {
            ...
            case .handlerError(_, let underlyingError):
                if let keychainError = underlyingError as? KeychainError {
                    return keychainError
                }
                return KeychainError.operationFailed("Handler error during \(operation): \(underlyingError)")
            case .cancelled:
                return KeychainError.operationCancelled("XPC operation cancelled during \(operation)")
            default:
                return KeychainError.operationFailed("XPC error during \(operation): \(error.localizedDescription)")
            }
...
/**
 Additional keychain-specific error types.
 */
