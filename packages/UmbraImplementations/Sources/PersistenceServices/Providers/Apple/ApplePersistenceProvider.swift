...
/**
 Provider implementation for Apple platforms with sandbox support.
 
 This provider implements the PersistenceProviderProtocol for Apple platforms,
 with full support for sandboxing requirements and security-scoped bookmarks.
 */
public class ApplePersistenceProvider: PersistenceProviderProtocol {
    /// Database URL
    ...
    /// File coordinator for coordinated file access
    ...
    private let accessQueue = DispatchQueue(label: "com.umbracore.persistence.apple", qos: .userInitiated)
    
    /**
     Initialises a new Apple persistence provider.
    ...
    public func create<T: Persistable>(item: T) async throws -> T {
        ...
        if var mutableDict = mutableItem as? [String: Any] {
            ...
            // This is a simplification - in a real implementation we'd need a better way to mutate the item
        }
        ...
        fileCoordinator.coordinate(
            ...
            do {
                try itemData.write(to: url, options: .atomic)
                writeSuccess = true
            } catch {
                writeSuccess = false
            }
    ...
    public func read<T: Persistable>(id: String, type: T.Type) async throws -> T? {
        ...
        // Use file coordinator for coordinated read
        var coordinatorError: NSError?
        var itemData: Data?
        ...
        fileCoordinator.coordinate(
            ...
            do {
                itemData = try Data(contentsOf: url)
            } catch {
                itemData = nil
            }
        }
        ...
        if let error = coordinatorError {
            throw PersistenceError.storageUnavailable(
                "Failed to read item: \(error.localizedDescription)"
            )
        }
        
        guard let data = itemData else {
            throw PersistenceError.storageCorruption(
                "Failed to read item data"
            )
        }
    ...
    public func update<T: Persistable>(item: T) async throws -> T {
        ...
        let existingItem = try await read(id: item.id, type: T.self)
        
        guard let existing = existingItem else {
            throw PersistenceError.itemNotFound(
                "Item with ID \(item.id) not found"
            )
        }
        ...
        if !writeSuccess {
            throw PersistenceError.general(
                "Failed to write updated item data"
            )
        }
    ...
    public func delete<T: Persistable>(id: String, type: T.Type) async throws -> Bool {
        // Create the file name for the item
        let fileName = "\(T.typeIdentifier)_\(id).json"
        let fileURL = databaseURL.appendingPathComponent(fileName)
        ...
        // Use file coordinator for atomic delete
        var coordinatorError: NSError?
    ...
    public func query<T: Persistable>(type: T.Type, options: QueryOptionsDTO) async throws -> [T] {
        ...
        let matchingFiles = fileURLs.filter { url in
            return url.lastPathComponent.hasPrefix(typePrefix) && url.pathExtension == "json"
        }
    ...
    public func count<T: Persistable>(type: T.Type, filter: [String: Any]?) async throws -> Int {
        // Get all files matching the type
        let typePrefix = "\(T.typeIdentifier)_"
    ...
    public func createBackup(options: BackupOptionsDTO) async throws -> BackupResultDTO {
        // Get a prepared backup directory
        let backupDir = try await prepareForBackup()
    ...
    public func restoreFromBackup(url: URL, password: String?) async throws -> Bool {
    ...
    public func getSecureFileURL(filename: String, inDirectory: String?, create: Bool) async throws -> URL {
    ...
    public func saveToSecureFile(data: Data, filename: String, inDirectory: String?) async throws -> URL {
        let fileURL = try await getSecureFileURL(filename: filename, inDirectory: inDirectory, create: true)
    ...
    public func readFromSecureFile(filename: String, inDirectory: String?) async throws -> Data {
        let fileURL = try await getSecureFileURL(filename: filename, inDirectory: inDirectory, create: false)
    ...
    // MARK: - Security Bookmark Management
    
    /**
     Registers a security-scoped bookmark for a URL.
     
     - Parameters:
        - url: The URL to create a bookmark for
        - isDirectory: Whether the URL points to a directory
     - Returns: The created bookmark data
     - Throws: PersistenceError if bookmark creation fails
     */
    public func registerSecurityBookmark(for url: URL, isDirectory: Bool) throws -> Data {
        guard url.startAccessingSecurityScopedResource() else {
            throw PersistenceError.permissionDenied(
                "Failed to access security-scoped resource: \(url.path)"
            )
        }
        ...
        let options: NSURL.BookmarkCreationOptions = isDirectory ? [.withSecurityScope, .securityScopeAllowOnlyReadAccess] : [.withSecurityScope]
    ...
    /**
     ...
     - Throws: PersistenceError if bookmark resolution fails
     */
    public func resolveSecurityBookmark(_ bookmarkData: Data) throws -> URL {
        ...
        do {
            ...
            if isStale {
                // In a real implementation, we would handle stale bookmarks
                // For simplicity, we'll just return the URL and log a warning
