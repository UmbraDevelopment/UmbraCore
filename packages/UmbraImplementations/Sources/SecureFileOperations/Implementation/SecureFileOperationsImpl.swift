...
public actor SecureFileOperationsImpl: SecureFileOperationsProtocol {
     ...
     */
    public func createSecurityBookmark(for path: FilePathDTO, readOnly: Bool) async throws -> (Data, FileOperationResultDTO) {
     ...
     */
    public func startAccessingSecurityScopedResource(at path: FilePathDTO) async throws -> (Bool, FileOperationResultDTO) {
    ...
    /**
     ...
     - Returns: True if access was granted, false otherwise, and operation result
     */
    public func accessSecurityScopedResource(at path: FilePathDTO) async -> (Bool, FileOperationResultDTO) {
        let context = createSecureFileLogContext([
            "path": path.path
        ])
     ...
     */
    public func stopAccessingSecurityScopedResource(at path: FilePathDTO) async -> FileOperationResultDTO {
        let context = createSecureFileLogContext(["path": path.path])
    ...
    public func createSecureTemporaryFile(prefix: String?, options: FileCreationOptions?) async throws -> FilePathDTO {
        ...
        fileManager.createFile(atPath: tempPath.path, contents: Data(), attributes: fileCreationOptions.attributes)
        ...
        if resourceValues.isDirectory == true {
            ...
            if passes > 0 {
                ...
                for pass in 1...passes {
                    ...
                    let data: Data
                    if useRandomData {
                        data = createRandomData(size: Int(fileSize))
                    } else {
