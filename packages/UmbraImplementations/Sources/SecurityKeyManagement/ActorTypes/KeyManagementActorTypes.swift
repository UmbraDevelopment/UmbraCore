...
extension KeyManagementError {
  ...
  public func toStandardError() -> SecurityProtocolError {
    switch self {
      ...
      case let .invalidInput(details):
        .inputError("Invalid input: \(details)")
      case let .keyManagementError(details):
        .operationFailed(reason: "Key management error: \(details)")
      case let .keyFormatError(details):
        .invalidMessageFormat(details: "Invalid key format: \(details)")
      case let .storageError(details):
        .operationFailed(reason: "Storage error: \(details)")
      case let .accessError(details):
        .authenticationFailed(reason: "Access error: \(details)")
      case let .keyInaccessible(details):
        .operationFailed(reason: "Key inaccessible: \(details)")
    }
  }
   ...
   */
  public static func fromStandardError(_ standardError: SecurityProtocolError)
  -> KeyManagementError {
