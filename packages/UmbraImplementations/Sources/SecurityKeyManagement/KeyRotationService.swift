...
public enum KeyRotationError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .rotationFailed(let id, let reason):
            return "Failed to rotate key \(id): \(reason)"
        case .invalidKeyIdentifier(let id):
            return "Invalid key identifier: \(id). Expected format is 'baseId_vN' where N is a version number."
        }
    }
}
...
public class KeyRotationServiceImpl: KeyRotationService {
    ...
    /**
     ...
     - Throws: `KeyRotationError.invalidKeyIdentifier` if the identifier
               cannot be parsed
     */
    internal func parseKeyIdentifier(_ identifier: String) throws -> (baseId: String, version: Int) {
...
public extension KeyRotationServiceImpl {
    /**
     ...
     */
    static func createDefault(keyStore: KeyStore, keyGenerator: KeyGenerator) -> KeyRotationService {
        return KeyRotationServiceImpl(keyStore: keyStore, keyGenerator: keyGenerator)
    }
}
