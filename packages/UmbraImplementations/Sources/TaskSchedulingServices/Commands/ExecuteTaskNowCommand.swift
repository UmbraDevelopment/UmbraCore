...
public class ExecuteTaskNowCommand: BaseTaskCommand, TaskCommand {
     ...
     */
    public func execute(context: LogContextDTO) async throws -> TaskExecutionResult {
        ...
        do {
            ...
            let completedTask: ScheduledTaskDTO
            if executionResult.success {
                completedTask = ScheduledTaskDTO(
                    id: task.id,
                    scheduleID: task.scheduleID,
                    name: task.name,
                    description: task.description,
            ...
            await logger.log(
                ...
                context: operationContext.withMetadata(
                    LogMetadataDTOCollection()
                        ...
                        .withPublic(key: "durationSeconds", value: String(format: "%.2f", endTime.timeIntervalSince(startTime)))
            ...
            return TaskExecutionResult(
                ...
                error: executionResult.success ? nil : TaskSchedulingError.executionFailed(executionResult.errorMessage ?? "Unknown error"),
                startTime: startTime,
                endTime: endTime
            )
    ...
    private func simulateBackupTask(
        ...
        if success {
            return InternalExecutionResult(
                success: true,
                outputData: [
                    "backupSize": "\(Int.random(in: 10000...1000000)) bytes",
    ...
    private func simulateRestoreTask(
        ...
        if success {
            return InternalExecutionResult(
                success: true,
                outputData: [
                    "restoredSize": "\(Int.random(in: 10000...1000000)) bytes",
    ...
    private func simulatePruneTask(
        ...
        if success {
            ...
            let spaceFreed = Int.random(in: 10000...10000000)
        ...
        case "data_sync":
            ...
            return InternalExecutionResult(
                ...
                outputData: [
                    ...
                    "bytesTransferred": "\(Int.random(in: 1000...1000000))",
