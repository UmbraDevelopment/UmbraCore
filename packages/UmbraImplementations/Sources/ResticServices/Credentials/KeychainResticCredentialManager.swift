...
/// A secure implementation of ResticCredentialManager that stores credentials in the system keychain
public actor KeychainResticCredentialManager: ResticCredentialManager {
    ...
    public func getCredentials(for repository: String) async throws -> ResticCredentials {
        await logger.debug("Retrieving credentials for repository", 
        ...
        do {
            ...
            throw ResticError.credentialError("Failed to retrieve credentials: \(error.localizedDescription)")
        }
    }
    ...
    public func storeCredentials(_ credentials: ResticCredentials, for repository: String) async throws {
        await logger.debug("Storing credentials for repository", 
                          metadata: ["repository": .string(repository)],
                          source: "KeychainResticCredentialManager")
        ...
        do {
            ...
            await logger.info("Successfully stored credentials for repository",
                             metadata: ["repository": .string(repository)],
                             source: "KeychainResticCredentialManager")
        } catch {
            await logger.error(
            ...
            throw ResticError.credentialError("Failed to store credentials: \(error.localizedDescription)")
        }
    }
    ...
    public func removeCredentials(for repository: String) async throws {
        await logger.debug("Removing credentials for repository", 
                          metadata: ["repository": .string(repository)],
                          source: "KeychainResticCredentialManager")
        ...
        do {
            ...
            await logger.info("Successfully removed credentials for repository",
                             metadata: ["repository": .string(repository)],
                             source: "KeychainResticCredentialManager")
        } catch {
            await logger.error(
            ...
            throw ResticError.credentialError("Failed to remove credentials: \(error.localizedDescription)")
        }
    }
    ...
    private func makeAccountIdentifier(for repository: String) -> String {
        ...
        return repository.trimmingCharacters(in: .whitespacesAndNewlines)
